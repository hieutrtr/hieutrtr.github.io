...
description: "Hai nền tảng mới định nghĩa lại cách build và deploy agent — nhưng trước khi đưa ra nhận định, đáng để xem một agent đang chạy thật có bao nhiêu layers, và nghiên cứu của chính Anthropic nói gì về độ phức tạp của harness."
featured: true
lang: vi
...
Nguyên tắc cốt lõi Anthropic nhấn mạnh: **bắt đầu ở lowest level còn hoạt động được và chỉ thêm phức tạp khi hiệu quả cải thiện đo được.** Nguyên tắc này cần nhớ khi đánh giá những platform mới mặc định chọn kiến trúc "agent đầy đủ".

---

## Stack Hiện Tại: Một Spectrum Từ Low Đến High
...
Hãy hình dung như một dial: từ "viết mọi thứ bằng code" đến "chỉ mô tả agent là gì":

**Lowest — Tự viết toàn bộ orchestration**
...
**Middle — Framework lo phần infrastructure**
...
**Higher — Platform lo deploy và vận hành**
...
*Ví dụ (Claude Managed Agents): Bạn cấu hình agent làm gì. Infrastructure của Anthropic tách Brain (Claude + controller logic) khỏi Hands (môi trường thực thi tạm thời, không truy cập được credential). Một append-only session log đóng vai trò persistent external memory — nếu Brain gặp sự cố giữa chừng, instance mới tiếp tục từ event cuối.*

**Highest — Chỉ cần system prompt, không cần infrastructure**
...
Cả hai platform đồng thuận về một điểm chung về kiến trúc: **danh tính và năng lực của agent phải tách biệt khỏi execution infrastructure**.

**Deep Agents Deploy của LangChain** hiện thực hóa điều này qua các open standards:
...
- **Brain** — Claude + controller logic. Stateless, thay thế được.
...
Loop Plan/Execute/Evaluate cho kết quả tốt — nhưng phát hiện thú vị hơn là những gì xảy ra khi underlying model cải thiện từ Claude Opus 4.5 lên 4.6.

> *"Mỗi thành phần trong một harness đều ẩn chứa một giả định về những gì model không thể tự làm, và những giả định đó đáng được kiểm tra lại."*

Khi Claude 4.6 xử lý tốt hơn với các tác vụ dài liền mạch, Anthropic đã đơn giản hóa harness: bỏ phân chia theo giai đoạn ngắn, chuyển đánh giá từ per-phase sang end-of-run. Orchestration trở nên ít phức tạp hơn, chứ không phải phức tạp thêm — vì model đã có thể xử lý nhiều hơn trực tiếp.
...
**Những gì chúng làm khó hơn hoặc tạo ra như trade-off mới:**
...
- Open question: các default của platform có khớp với nhu cầu thực của agent bạn không
...
Cả Managed Agents lẫn Deep Agents Deploy đều chia sẻ một concept cốt lõi: định nghĩa agent ở context layer — danh tính, kỹ năng, kiến thức trong các file có cấu trúc, không phải trong code. OpenClaw, một agent framework do cộng đồng phát triển, đã đến cùng nhận định kiến trúc này theo con đường riêng.
...
Tháng 2/2026, một cuộc tấn công supply chain nhắm vào ClawHub (gọi là "ClawHavoc") được tiết lộ. Kẻ tấn công đã compromise 12 tài khoản publisher và đăng tải 1.184 skill độc hại. Một khi được nạp vào context của agent, các skill này có thể leak credential, redirect tool call, hoặc làm nhiễm reasoning của agent. Cuộc tấn công đã hoạt động trong một khoảng thời gian chưa xác định trước khi bị phát hiện.
...
**Task decomposition vẫn quan trọng.** Không platform nào tự động xử lý được task mơ hồ hoặc thiếu thông tin. Instruction cho agent vẫn cần rõ ràng về what done looks like. `AGENTS.md` chỉ hữu ích như cái suy nghĩ đã đi vào việc viết nó.

**Evaluation vẫn là việc của bạn.** Managed Agents sinh session log. Deep Agents tích hợp LangSmith. Nhưng quyết định agent có làm đúng không — và chẩn đoán silent failures — vẫn là trách nhiệm của bạn.
...
Điều đáng ghi nhớ hơn từ cả hai bản phát hành, và từ nghiên cứu kỹ thuật harness của Anthropic, là về kiến trúc: **danh tính và năng lực của agent thuộc về một portable definition layer, dễ đọc; execution, memory, security, và deployment machinery thuộc về infrastructure bên dưới nó.** Dù bạn đến đó qua managed platform hay tự build, sự phân tách đó là đáng hướng tới.

...
Sự tiến triển của Anthropic — Augmented LLM → Workflows → Agent tự chủ — mô tả *độ phức tạp* của agent, không phải lựa chọn platform. Một managed platform hoàn toàn có thể chạy một agent Level 1. Một custom LangGraph harness hoàn toàn có thể implement các mẫu Level 3. Hai trục này độc lập với nhau.
